...
public class Worker extends AbstractLoggingActor {
    ...
    private void handle(Master.PasswordCrackRequest message) {
        char[] chars = message.getCharSet().toCharArray();
        this.calculatePermutationsAndCheckIfPassword(chars, "", chars.length, message.getPasswordLength(), message.getPassword());
        this.sender().tell(new FoundPasswordMessage(message.getPassword(), this.cleanPassword), this.self()); // this can act as gimme work message at the same time
    }
